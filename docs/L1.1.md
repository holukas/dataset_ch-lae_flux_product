...
The calculated $\text{FCT}_{\text{unsc}}$ is derived from theoretical/empirical models, but it requires tuning for a specific instrument and site. The `ScopOptimizer` class handles this critical step, using parallel measurements from an immune **Enclosed-Path (CP) IRGA** as the true flux reference.
