...
file_fluxes_meteo <- "13_SUBSET_NEE_QCF11_IRGA72_2016-2024.csv"
output_path <- getwd()
Sys.setenv(TZ = "GMT")
...
EddyData.F <- subset (EddyData.F, TIMESTAMP >= as.POSIXct('2016-01-01 00:30:00'))  # Date with first fluxes
EddyData.F <- subset (EddyData.F, TIMESTAMP <= as.POSIXct('2025-01-01 00:00:00'))
...
EddyProc.C$sEstimateUstarScenarios(
  ...
  ctrlUstarEst = usControlUstarEst(
    ...
    ustPlateauFwd = 9,  # (default: 10) number of subsequent uStar bin values to compare to in fwd mode
    ustPlateauBack = 5,  # (default: 6) number of subsequent uStar bin values to compare to in back mode
...
write.csv(uStarTh, file = paste("16_USTAR-THRESHOLDS_NEE_QCF11_IRGA72_2016-2024_RP-", run_id,".csv",sep=""))
